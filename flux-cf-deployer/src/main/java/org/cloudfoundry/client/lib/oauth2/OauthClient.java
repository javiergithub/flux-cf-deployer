...
//This code copied from the CloudFoundry Java client. Can't use it directly as dependency
// because conflicts with other spring boot dependencies.
// So only copied some bits and pieces that we use.
...
public class OauthClient {
	...
	public OAuth2AccessToken getToken() {
		...
		if (token.getExpiresIn() < 30*60) { // original was 50 seconds before expiration => Then refresh it.
										// Changed because we want to avoid sending 'refresh token' 
										// to the CF deployer service. We only send the
										// access token. Deployer may need to execute multiple successive
										// and fairly lengthy operations. So 50 seconds may not be enough. 
										// Therefore we changed it to 30 minutes.
